		...
		if (parseInt(table) >= 1 && parseInt(table) <= 10){
			tableinfo.find({table:table}).then((docs)=>{
				if (docs[0]["status"] != "1" && docs[0] != ""){
					...
					res.json([{"URL":'/toorder', "data":{"table":table,"session":docs[0]["sessionCode"]}}]);
				}
			}).catch((error)=>{
				res.json(error);
			})
	...
	if (check.length != 0 && check[0]['loginCookies'] != '0'){	
		...
		userHistory.find({userName:userName[0]['userName']}).then(async (docs)=>{
			for (let i = 0;i < docs.length; i++){
				...
				send.push({"historyId":docs[i]['_id'],"foodName":docs2[0]['foodName'], "description":docs2[0]['description'],"Price":docs2[0]['price'], "time":docs[i]['time']});
			}
			res.json(send);
...
router.post("/askhealthquestion", express.urlencoded({ extended: true }), async (req, res) => {
	...
	if (check.length != 0 && check[0]['loginCookies'] != '0'){
		...
		const response = await openai.createChatCompletion({
			model: "gpt-3.5-turbo",
			messages: [{ "role": "user", "content": 'Act as an AI health assistant and reply to me in 50 words, if the following question is not about health just reply to me "I will only reply to you with a health question! question:" '+question }],
			})
			...
			if (data.length != 0){
				...
				try{
					const response = await openai.createChatCompletion({
						model: "gpt-3.5-turbo",
						messages: [
							{ "role": "user", "content": 'Analysis the data for me and reply me with a health suggestion in 50 words, 1=Grains 2=Vegetables 3=Fruits 4=Meat, fish, egg and alternatives 5= milk and alternatives 6=food and drinks with high Fat/oil, salt and sugar, data:' +str}],
						})
